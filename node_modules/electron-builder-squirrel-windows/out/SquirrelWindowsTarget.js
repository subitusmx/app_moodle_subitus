    ...
    computeEffectiveDistOptions() {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (iconUrl == null) {
                ...
                if (iconUrl == null) {
                    throw new (_builderUtil || _load_builderUtil()).InvalidConfigurationError("iconUrl is not specified, please see https://electron.build/configuration/configuration#WinBuildOptions-iconUrl");
                }
            }
...
function checkConflictingOptions(options) {
    for (const name of ["outputDirectory", "appDirectory", "exe", "fixUpPaths", "usePackageJson", "extraFileSpecs", "extraMetadataSpecs", "skipUpdateIcon", "setupExe"]) {
        if (name in options) {
            throw new (_builderUtil || _load_builderUtil()).InvalidConfigurationError(`Option ${name} is ignored, do not specify it.`);
        }
    }
    if ("noMsi" in options) {
    ...
    if (msi != null && typeof msi !== "boolean") {
        throw new (_builderUtil || _load_builderUtil()).InvalidConfigurationError(`msi expected to be boolean value, but string '"${msi}"' was specified`);
    }
}
